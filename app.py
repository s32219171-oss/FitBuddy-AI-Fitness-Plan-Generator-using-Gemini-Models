...
st.info("Enter your fitness details and Gemini AI will generate a personalized fitness plan.")
...
goal = st.selectbox(
    ...
    ["Weight Loss", "Muscle Building", "General Fitness",
     "Strength Improvement", "Flexibility"]
)
...
if st.button("🤖 Generate My AI Fitness Plan", use_container_width=True):

    if client is None:
        st.error("Gemini API key is not configured. Please check Streamlit Secrets.")
...
"""
        ...
        try:
            with st.spinner("🤖 Gemini is creating your personalized plan..."):
            ...
            st.success("✅ AI Fitness Plan Generated Successfully!")
            ...
            st.error("Unable to generate the AI fitness plan.")
            st.caption(f"Error: {e}")

st.markdown("---")
st.caption("🏋️ FitBuddy – AI Fitness Plan Generator using Gemini Models | College Project")
